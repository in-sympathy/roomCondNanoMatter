/*
  ...
  This version:
  - Color-coded alerts, 3 blinks per issue (sequenced if multiple):
      eCO2  -> Blue
      tVOC  -> Orange (R+G)
      Humid -> Purple (R+B)
      Press -> Purple (R+B)
      Temp  -> Red
    If no RGB LED is present, falls back to a single LED and still sequences 3 blinks per issue.
  - OLED shows AQI label ("Excellent/Good/Fair/Poor/Very Poor") + numeric value.
  - Pairing layout, QR fit, brightness, and no-flicker behavior preserved.
...
// If your RGB LED or alert LED is active-LOW, set these to 0.
#ifndef LED_RGB_ACTIVE_HIGH
  #define LED_RGB_ACTIVE_HIGH 1
#endif
...
// Blink timings
const uint16_t BLINK_ON_MS   = 200;
const uint16_t BLINK_OFF_MS  = 200;
const uint16_t GROUP_GAP_MS  = 350;

// Sequencer runtime state
uint8_t  alertList[5] = {0};
uint8_t  alertCount   = 0;
uint8_t  curIssueIdx  = 0;
uint8_t  curBlink     = 0;     // 0..2 (3 blinks)
bool     phaseOn      = true;  // ON or OFF phase within a blink
bool     inGroupGap   = false; // gap between issues
uint32_t lastPhaseMs  = 0;
uint8_t  prevMask     = 0;

// Map issue -> RGB color
static inline void colorForIssue(Issue is, bool &r, bool &g, bool &b) {
  switch (is) {
    case ISSUE_ECO2: r = false; g = false; b = true;  break;           // Blue
    case ISSUE_TVOC: r = true;  g = true;  b = false; break;           // Orange (R+G)
    case ISSUE_HUM:  r = true;  g = false; b = true;  break;           // Purple (R+B)
    case ISSUE_PRS:  r = true;  g = false; b = true;  break;           // Purple (R+B)
    case ISSUE_TMP:  r = true;  g = false; b = false; break;           // Red
    default:         r = g = b = false; break;
  }
}

// Rebuild active-issues list and mask
static inline uint8_t buildAlerts(bool eco2_bad, bool tvoc_bad, bool hum_bad, bool prs_bad, bool tmp_bad) {
  alertCount = 0;
  if (eco2_bad) alertList[alertCount++] = ISSUE_ECO2;
  if (tvoc_bad) alertList[alertCount++] = ISSUE_TVOC;
  if (hum_bad)  alertList[alertCount++] = ISSUE_HUM;
  if (prs_bad)  alertList[alertCount++] = ISSUE_PRS;
  if (tmp_bad)  alertList[alertCount++] = ISSUE_TMP;
  uint8_t m = (eco2_bad?1:0) | (tvoc_bad?2:0) | (hum_bad?4:0) | (prs_bad?8:0) | (tmp_bad?16:0);
  return m;
}

// Non-blocking LED sequencer: 3 blinks per active issue, cycle through all
void updateAlertLed(uint32_t ms) {
  ...
  uint8_t mask = buildAlerts(eco2_bad, tvoc_bad, hum_bad, prs_bad, tmp_bad);

  if (mask == 0) {
    // No alerts
    setRgb(false,false,false);
    // Reset sequence so next alert starts cleanly
    curIssueIdx = 0; curBlink = 0; phaseOn = true; inGroupGap = false;
    prevMask = 0;
    return;
  }

  // If the set of active issues changed, restart the sequence at the first one
  if (mask != prevMask) {
    curIssueIdx = 0; curBlink = 0; phaseOn = true; inGroupGap = false;
    lastPhaseMs = ms; // start immediately
    prevMask = mask;
  }

  // Current issue color
  bool r=false,g=false,b=false;
  colorForIssue((Issue)alertList[curIssueIdx], r,g,b);

  if (inGroupGap) {
    // Inter-issue gap (LED off)
    setRgb(false,false,false);
    if (ms - lastPhaseMs >= GROUP_GAP_MS) {
      inGroupGap = false;
      curBlink = 0;
      phaseOn = true;
      lastPhaseMs = ms;
      setRgb(r,g,b); // start next issue ON
    }
    return;
  }
  ...
  if (phaseOn) {
    // LED ON phase
    setRgb(r,g,b);
    if (ms - lastPhaseMs >= BLINK_ON_MS) {
      phaseOn = false;
      lastPhaseMs = ms;
      setRgb(false,false,false);
    }
  } else {
    // LED OFF phase within a blink
    if (ms - lastPhaseMs >= BLINK_OFF_MS) {
      curBlink++;
      if (curBlink >= 3) {
        // Done with this issue -> gap, then move to next
        inGroupGap = true;
        lastPhaseMs = ms;
        setRgb(false,false,false);
        curIssueIdx++;
        if (curIssueIdx >= alertCount) curIssueIdx = 0; // loop
      } else {
        // Next ON phase of same issue
        phaseOn = true;
        lastPhaseMs = ms;
        setRgb(r,g,b);
      }
...
void loop() {
  ...
  // Color-coded alert sequencer every loop
  updateAlertLed(ms);
}

/*
  ...
  ALSO RECOMMENDED (outside this sketch): update the Silicon Labs core to
  >= 3.0.0 (ideally 4.0.0). 3.0.0 brings the Matter 1.4 stack with more
  robust reconnections and bundles a radio-safe OneWire fork (remove the
  stock OneWire library from your libraries folder so the bundled one is
  used); 4.0.0 fixes DataVersion size passing in the Matter library, part of
  the attribute-subscription mechanism controllers use to receive reports.
*/
